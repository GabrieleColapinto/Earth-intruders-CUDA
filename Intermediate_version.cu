...
/*
	In this version of the project the kernels which effect a grid are divided
	in more than one block.
	The kernels which affect only one row or column are not changed.

	The following kernels are now used differently to
	divide the rows among the blocks and refer to the cells of the
	matrix using the threads.
		- destroyerCollateralEffect
		- initCurand

	The changed kernels are:
		- matrixInitializer (We now do everything in parallel without iterations)
		- collateralEffectsActivator (We now do everything in parallel without iterations)
		- advance (It now uses a result matrix)
		- reconversion (We now do everything in parallel in more than one block)
*/
...
/*
	...
	In the advanced version (task 8) every row of the matrix goes in
	a different block and the cells of the rows are handled by the
	threads.
*/
...
__global__ void advance(char* dev_Stage_Matrix, char* dev_Result_Matrix, int* dev_Support_Matrix, int* dev_turn_score, int* dev_turn_lives, bool* dev_advanceDamage, int numFilas, int numColumnas) {
	int row = blockIdx.y * blockDim.y + threadIdx.y;
	int col = blockIdx.x * blockDim.x + threadIdx.x;

	if (col < numColumnas && row < numFilas) {
		/*
		  This function analyzes the stage matrix and generates the result matrix.
		  After waiting for all the threads to generate the result matrix we can
		  copy the result matrix into the stage matrix in one turn.
		  Using this method we avoid using a for loop to elaborate the matrix.
		...
		*/
		if (row == 0) {
			if (dev_Stage_Matrix[col] == cell_letters[player]) {
				dev_Result_Matrix[col] = cell_letters[player];
				//Damage check
				if (dev_Stage_Matrix[numColumnas + col] != cell_letters[empty]) {
				...
				*/
				dev_Result_Matrix[col] = dev_Stage_Matrix[numColumnas + col];
			}
		}
		else {
			if (row == SHIELD_ROW - 1) {
				...
				if (dev_Stage_Matrix[SHIELD_ROW * numColumnas + col] == cell_letters[shield]) {
					dev_Result_Matrix[row * numColumnas + col] = cell_letters[empty];
				}
				else {
					dev_Result_Matrix[row * numColumnas + col] = dev_Stage_Matrix[SHIELD_ROW * numColumnas + col];
				}
			}
			else {
				if (row == SHIELD_ROW) {
					...
					*/
					if (dev_Stage_Matrix[SHIELD_ROW * numColumnas + col] != cell_letters[shield]) {
						// Advance
						dev_Result_Matrix[SHIELD_ROW * numColumnas + col] = dev_Stage_Matrix[(SHIELD_ROW + 1) * numColumnas + col];
					}
					else {
						if (dev_Stage_Matrix[(SHIELD_ROW + 1) * numColumnas + col] == cell_letters[destroyer]) {
							...
							dev_Result_Matrix[SHIELD_ROW * numColumnas + col] = cell_letters[shield];
							...
							if (dev_Stage_Matrix[(SHIELD_ROW + 1) * numColumnas + col] == cell_letters[cruiser]) {
								...
								dev_Result_Matrix[SHIELD_ROW * numColumnas + col] = cell_letters[empty];
					...
					if (row == numFilas - 1) {
						...
						*/
						dev_Result_Matrix[row * numColumnas + col] = dev_Stage_Matrix[(row + 1) * numColumnas + col];
					}
				}
			}
		}
		...
		/*
			...
			the result matrix and copy its content back into the stage matrix.
			We can do it entirely in parallel.
		*/
		__syncthreads();
		dev_Stage_Matrix[row * numColumnas + col] = dev_Result_Matrix[row * numColumnas + col];
	}
}
...
int main() {
	...
	//Result matrix of the kernels
	char* h_Result_Matrix = (char*)malloc(MAIN_MATRIX_SIZE);
	char* dev_Result_Matrix;
	...
	cudaMalloc(&dev_Result_Matrix, MAIN_MATRIX_SIZE);
	...
	cudaMemcpy(dev_Result_Matrix, h_Result_Matrix, MAIN_MATRIX_SIZE, cudaMemcpyHostToDevice);
	...
	matrix_initializer << <MATRIX_BLOCKS, MATRIX_THREADS >> > (dev_Stage_Matrix, dev_Support_Matrix, initial_player_position, numFilas, numColumnas);
	...
	while (lives > 0) {
		...
		reconversion << <MATRIX_BLOCKS, MATRIX_THREADS >> > (dev_Stage_Matrix, dev_Support_Matrix, d_state, numFilas, numColumnas);
		advance << <MATRIX_BLOCKS, MATRIX_THREADS >> > (dev_Stage_Matrix, dev_Result_Matrix, dev_Support_Matrix, dev_turn_score, dev_turn_lives, dev_advance_damage, numFilas, numColumnas);
		collateralEffectsActivator << <COLLATERAL_BLOCKS, MATRIX_THREADS >> > (dev_Stage_Matrix, dev_Support_Matrix, numFilas, numColumnas, d_state, dev_destroyer_collateral_damage, dev_cruiser_collateral_damage);
		addAliens << <1, numColumnas >> > (dev_Stage_Matrix, d_state, numFilas, numColumnas);
